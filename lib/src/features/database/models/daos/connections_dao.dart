...
/// A database accessor object for [Connections] related interactions.

@DriftAccessor(tables: [Connections])
class ConnectionsDao extends DatabaseAccessor<Database>
    with _$ConnectionsDaoMixin {
  /// A database accessor object for [Connections] related interactions.
  ConnectionsDao(super.attachedDatabase);

  /// Insert [connection] into the database.
  Future<int> insertConnection(Connection connection) async {
    final id = await managers.connections.create(
      (_) => connection,
    );
    Logger.instance.i('Inserted connection $id.');
    return id;
  }
}
