...
class ABTrackingMenu extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final workSessionGuidanceActive =
        ref.watch(activeWorkSessionProvider.select((value) => value != null)) &&
        ref.watch(
    ...
    return MenuButtonWithChildren(
      ...
      menuChildren: [
                      ...
                      .update(
                        ref.read(
                          displayABTrackingProvider.select(
                            (value) =>
                                value?.baseLine.map((e) => e.position).toList(),
                          ),
                        ),
                      );
                },
        ...
        if (ref.watch(enableSelectablePathProvider) &&
            ref.watch(
              selectablePathIndicesProvider.select(
...
class _APlusLineBearingDialogState
  ...
  void initState() {
    super.initState();
    bearing =
        ref.read(aPlusLineProvider).value?.initialBearing ??
      ...
      children: [
        ...
        if (ref.watch(enableDebugModeProvider)) ...[
          ...
          if (ref.watch(debugABTrackingProvider)) ...[
            ...
            Consumer(
              builder: (context, ref, child) {
                final numPointsBehind = ref.watch(
                  aBDebugNumPointsBehindProvider,
                );
                return Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
